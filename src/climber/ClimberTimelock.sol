...
contract ClimberTimelock is ClimberTimelockBase {
    ...
    function schedule(
        ...
        if (targets.length == MIN_TARGETS || targets.length >= MAX_TARGETS) revert InvalidTargetsCount();
        ...
        if (getOperationState(id) != OperationState.Unknown) revert OperationAlreadyKnown(id);
    ...
    function execute(address[] calldata targets, uint256[] calldata values, bytes[] calldata dataElements, bytes32 salt) external payable {
        ...
        if (getOperationState(id) != OperationState.ReadyForExecution) revert NotReadyForExecution(id);

        operations[id].executed = true;
    }

    function updateDelay(uint64 newDelay) external {
        if (msg.sender != address(this)) revert CallerNotTimelock();
