...
contract FreeRiderNFTMarketplace is ReentrancyGuard {
    ...
    function _buyOne(uint256 tokenId) private {
        ...
        //@audit Should deduct the msg.value by priceToPay! 
        
        amountOfOffers--;
        
        //@audit Transfers the NFT and then pays the owner, should be reversed!
        // transfer from seller (martket) to buyer (attacker contract)
        token.safeTransferFrom(token.ownerOf(tokenId), msg.sender, tokenId);

        // pay current owner (our attacker contract!)
